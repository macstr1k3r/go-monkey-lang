...
func TestReturnStatements(t *testing.T) {
	...
	if len(program.Statements) != 3 {
		fmt.Println("We don't have 3 statements parsed")
		t.FailNow()
	}
	...
	for _, stmt := range program.Statements {
		...
		if !ok {
			fmt.Println("We didn't parse out a return statement")
			t.FailNow()
		}

		if retStmt.TokenLiteral() != "return" {
			fmt.Println("The parsed return statement does not have the correct token literal")
			t.FailNow()
		}
	}
...
func TestIdentifierExpression(t *testing.T) {
	...
	if ident.TokenLiteral() != "identifier" {
		t.Errorf("ident.TokenLiteral not %s. got=%s", "identifier",
			ident.TokenLiteral())
	}
}
...
func TestParsingPrefixExpressions(t *testing.T) {
	...
	for _, test := range prefixTests {
		...
		if !testIntegerLiteral(t, exp.Right, test.val) {
			return
		}
